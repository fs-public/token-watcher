...
const useDaiData = () => {
    ...
    const { fromFilter, toFilter } = useSelector(
    ...
    const fetchTransfers = async (
        ...
        try {
            ...
            let uniqueBlocknumbers: number[] = _data
                ...
                .filter(
                    (blockNumber, i, arr) => arr.indexOf(blockNumber) === i
                );

            let promises: Promise<BlockHeader>[] = uniqueBlocknumbers.map(
                (blockNumber: number) => web3.eth.getBlock(blockNumber, false)
            );
