...
const useWeb3 = () => {
    ...
    const [dai] = useState<Contract>(
        new web3.eth.Contract(DaiAbi, DAI_ADDRESS)
    );
    ...
    const getInitialEventsInfura = async (
        ...
        const getEventsRecursive = async (
            ...
            if (
                (middle === 0 && !tooManyResults) ||
                events.length >= desiredEntries
            ) {
                ...
                if (tooManyResults) {
                    await getEventsRecursive(
                        middle,
                        Math.ceil((middle + end) / 2),
                        end
                    );
                } else {
                    await getEventsRecursive(
                        start,
                        Math.ceil((start + middle) / 2),
                        middle
                    );
                }
            }
