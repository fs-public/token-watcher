...
export const applicationSlice = createSlice({
  ...
  reducers: {
    setFilter: (
      ...
      if (action.payload.filterRule !== "" && !Web3.utils.isAddress(action.payload.filterRule)) {
        throw new Error("Specified address is not a valid address.")
      }

      if (action.payload.fieldKey === "from") state.fromFilter = action.payload.filterRule
      else if (action.payload.fieldKey === "to") state.toFilter = action.payload.filterRule
    },
...
export const { setFilter, setEmitterFromBlock, sortClicked } = applicationSlice.actions
